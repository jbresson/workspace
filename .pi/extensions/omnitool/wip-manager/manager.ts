...
/**
 * When true: new wip worktrees use git sparse-checkout (only changed files checked out).
...
export class WipWorktreeManager {
  ...
  async wipPrepare(params: { project: string; issue: string; slug: string; base?: string }) {
    try {
      ...
      if (exists) {
        ...
        if (SPARSE_CHECKOUT) {
          // Sparse: create worktree without checking out files, then configure sparse-checkout
          const add = await this.runGit(canonicalPath, ["worktree", "add", "--no-checkout", "-b", sourceBranch, worktreePath, baseRef]);
          if (add.code !== 0) throw new Error(add.stderr || "WIP_E_INTERNAL");
          await this.runGit(worktreePath, ["sparse-checkout", "init", "--cone"]);
          // Caller responsible for setting paths via: git sparse-checkout set <paths>
        } else {
  ...
  async wipGraduate(params: { project: string; issue: string; slug: string; to: string; noSquash?: boolean; description?: string; issueLink?: string; noPrune?: boolean }) {
    ...
    try {
      ...
      if (txId) await this.releaseLock();
  ...
  async wipSeed(params: { project: string; issue: string; slug: string; paths: string[]; overwrite?: boolean }) {
    try {
      ...
      for (const rel of params.paths) {
        ...
        const r = await this.exec("cp", params.overwrite ? ["-f", src, dst] : ["-n", src, dst]);
        if (r.code === 0) copied.push(rel);
      }
...
export async function executeWipSubAction(
  ...
  const workspaceRoot = path.dirname(wipRoot);
  const { subAction } = params;

  // Git worktree operations — require pi API closures
  if (["prepare", "list", "status", "graduate", "prune", "seed"].includes(subAction)) {
    if (!pi) return { content: [{ type: "text", text: `subAction '${subAction}' requires pi API (exec/readFile/writeFile)` }], isError: true };
    const mgr = new WipWorktreeManager(workspaceRoot, pi.exec, pi.readFile, pi.writeFile);
    switch (subAction) {
      case "prepare":  return mgr.wipPrepare(params);
      case "list":     return mgr.wipList(params);
      case "status":   return mgr.wipStatus(params);
      case "graduate": return mgr.wipGraduate(params);
      case "prune":    return mgr.wipPrune(params);
      case "seed":     return mgr.wipSeed(params);
    }
  }
