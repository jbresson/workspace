...
describe('Gatekeeper hardening policy', () => {
    ...
    beforeEach(() => {
        ...
        config_1.GuardrailConfig.getInstance().setMode(config_1.GuardrailMode.ENFORCE);
        expectationServiceStub.findActive.mockClear();
        gatekeeper = new gatekeeper_1.Gatekeeper(expectationServiceStub);
    });
    afterEach(() => {
    ...
    it('enforces coverage for every active guardrail rule', () => {
        const activeRuleIds = gatekeeper_rules_1.GLOBAL_RULES.map((r) => r.id).sort();
