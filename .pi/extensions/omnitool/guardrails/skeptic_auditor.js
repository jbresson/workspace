...
class SkepticAuditor {
     ...
     */
    async audit(expectation, proof, projectContext) {
...
If you find any flaw, reject it and specify what is missing.`;
        ...
        const result = await llm_service_1.LLMService.call("SKEPTIC", prompt, context);
        if (result.isError) {
            return {
                approved: false,
     ...
     */
    async proposeValidator(expectation) {
...
The validator must be objective and not rely on hardcoded success signals.`;
        const result = await llm_service_1.LLMService.call("SKEPTIC", prompt);
        return result.isError ? 'echo "error proposing validator"' : result.text;
    }
    /**
     * Evaluates a tweak to a proposed validator.
     */
    async evaluateTweak(currentValidator, tweak) {
...
exports.SkepticAuditor = SkepticAuditor;
