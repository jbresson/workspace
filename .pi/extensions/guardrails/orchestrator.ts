...
export class GuardrailOrchestrator {
   ...
   */
  async handleAction(target: string, sessionId: string): Promise<{ allowed: boolean, exp?: any }> {
    try {
      const result = await this.gatekeeper.intercept(target);
      if (result.allowed) return { allowed: true };

      // If blocked, we check if we are in a negotiation phase
      const pending = result.expectations[0];
      return { allowed: false, exp: pending };
    } catch (error) {
